# Version 1.3.6 - CSV Import Integration & Bidirectional Sync
...
- API automatically handles:
  - Email → Entra Object ID (GUID) conversion
...
- Parses UI format with intelligent field mapping:
  ...
  - `Acronym` → `acronyms`
  - `Resources` (name:url) → structured resource objects
  - `Experts`/`Stewards` (email:info) → `owner_ids`
...
### 2. Bidirectional Glossary Sync - Classic ↔ Unified Catalog
...
**Direction**: Classic Glossary → Unified Catalog
...
- Maps classic term fields to UC structure:
  - `longDescription`/`shortDescription` → `description`
...
|---------|-----------|----------|
| `pvw uc term sync-classic` | UC → Classic | Publish business metadata to traditional glossaries |
| `pvw glossary sync-uc` | Classic → UC | Migrate classic terms to governance domains |
...
4. ✅ Terms migrated to governance domain
...
4. ✅ Terms created with full metadata (hierarchy, relationships, etc.)
...
- Fixed `includeTermHierarchy` parameter default (False → True)
