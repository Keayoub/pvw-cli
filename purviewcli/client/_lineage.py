...
class CSVLineageProcessor:
    ...
    def __init__(self, purview_client):
        ...
        self.account_name = purview_client.account_name if hasattr(purview_client, 'account_name') else None
...
class Lineage(Endpoint):
    ...
    def lineageReadUniqueAttribute(self, args):
        """Read lineage information for an entity by unique attribute (Official API)"""
        self.method = 'GET'
        self.endpoint = PurviewEndpoints.format_endpoint(
            PurviewEndpoints.LINEAGE['unique_attribute'], 
    ...
    def lineageBulkUpdate(self, args):
        """Update multiple lineage relationships in bulk (Official API)"""
        self.method = 'PUT'
    ...
    def lineageReadDownstream(self, args):
        """Get downstream lineage for an entity (Official API)"""
        self.method = 'GET'
        self.endpoint = PurviewEndpoints.format_endpoint(PurviewEndpoints.LINEAGE['downstream'], guid=args["--guid"])
        self.params = {
            'depth': args.get('--depth', 3),
            'width': args.get('--width', 6),
            'includeParent': 'true',
    ...
    def lineageReadUpstream(self, args):
        """Get upstream lineage for an entity (Official API)"""
        self.method = 'GET'
        self.endpoint = PurviewEndpoints.format_endpoint(PurviewEndpoints.LINEAGE['upstream'], guid=args["--guid"])
        self.params = {
            'depth': args.get('--depth', 3),
            'width': args.get('--width', 6),
            'includeParent': 'true',
    ...
    def lineageCreateRelationship(self, args):
        """Create a new lineage relationship (Official API)"""
        self.method = 'POST'
    ...
    def lineageUpdateRelationship(self, args):
        """Update an existing lineage relationship (Official API)"""
        self.method = 'PUT'
        self.endpoint = PurviewEndpoints.format_endpoint(PurviewEndpoints.LINEAGE['guid'], guid=args["--guid"])
        self.payload = get_json(args, '--payloadFile')
    ...
    def lineageDeleteRelationship(self, args):
        """Delete a lineage relationship (Official API)"""
        self.method = 'DELETE'
        self.endpoint = PurviewEndpoints.format_endpoint(PurviewEndpoints.LINEAGE['guid'], guid=args["--guid"])
        self.params = {
    ...
    def lineageGetMetrics(self, args):
        ...
        self.method = 'GET'
        self.endpoint = f'{PurviewEndpoints.DATAMAP_BASE}/{PurviewEndpoints.ATLAS_V2}/lineage/metrics'
        self.params = {
