...
# Add custom options for collections import and export commands
collections_group = main.get_command(None, "collections")
if collections_group:
    import_cmd = collections_group.get_command(None, "import")
    if import_cmd:
        import_cmd = click.option("--csvfile", required=True, help="CSV file path for import")(
            import_cmd
        )
        import_cmd = click.option("--batchsize", default=10, help="Batch size for processing")(
            import_cmd
        )

    export_cmd = collections_group.get_command(None, "export-csv")
    if export_cmd:
        export_cmd = click.option("--outputfile", help="Output CSV file path")(export_cmd)
        export_cmd = click.option(
            "--include-hierarchy", is_flag=True, default=True, help="Include hierarchy information"
        )(export_cmd)
        export_cmd = click.option(
